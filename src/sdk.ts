...
import type { StructaPrograms } from "./constants";
import {
 ...
 * StructaSDK.
 */
export class StructaSDK {
  constructor(
    readonly provider: AugmentedProvider,
    readonly programs: StructaPrograms
  ) {}

  /**
   * Creates a new instance of the SDK with the given keypair.
   */
  withSigner(signer: Signer): StructaSDK {
    return StructaSDK.load({
   ...
   * @returns
   */
  static load({ provider }: { provider: Provider }): StructaSDK {
    const programs: StructaPrograms = newProgramMap<StructaPrograms>(
      provider,
      STRUCTA_IDLS,
      STRUCTA_ADDRESSES
    );
    return new StructaSDK(new SolanaAugmentedProvider(provider), programs);
  }
